...
namespace DidiFrame.Entities.FluentValidation
{
	internal class MessageEmbedValidator : AbstractValidator<MessageEmbed>
	{
		public MessageEmbedValidator()
		{
			...
			RuleFor(s => s.Metadata.AuthorName).NotEmpty().MaximumLength(256).When(s => s is not null);
			RuleFor(s => s.Metadata.AuthorIconUrl).Must((obj, s) => obj.Metadata.AuthorName is not null).When(s => s is not null);
			RuleFor(s => s.Metadata.AuthorPersonalUrl).Must((obj, s) => obj.Metadata.AuthorName is not null).When(s => s is not null);
