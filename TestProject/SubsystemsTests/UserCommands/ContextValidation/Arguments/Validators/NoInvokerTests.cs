...
{
	public class NoInvokerTests
	{
		public NoInvoker CreateInstance()
		{
			return new NoInvoker();
		}

		[Test]
		public void ArgumentIsInvoker()
		{
			...
			var commandContext = new ValidatedUserCommandContext(new(new UserCommandSendData(member, textChannel), command,
				...
				{ { argument, new(argument, member, new[] { member }) } }, SimpleModelAdditionalInfoProvider.Empty));

			var validator = CreateInstance();

			//-------------------

			var workResult = validator.Validate(commandContext, new(argument, member, new[] { member }), EmptyServiceProvider.Instance);

			//-------------------

			Assert.That(workResult, Is.Not.Null);
			Assert.That(workResult.ErrorCode, Is.EqualTo("MemberIsInvoker"));
		}

		[Test]
		public void ArgumentIsNotInvoker()
		{
			var client = new Client();
			var server = client.CreateServer();
			var member = server.AddMember("Who?", false, Permissions.All);
